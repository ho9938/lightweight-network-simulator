...
class SEN:
    ...
    def route(self, flit):
        ...
        if srcidx == dstidx:
            return None
        elif aux <= self.n and srcidx[-1] == dstidx[aux-1]:
            flit.aux = aux # rol
...
class SEN_DF:
    ...
    def route(self, flit):
        ...
        if isinstance(src, Node):
            ...
            _srcidx = src.name[3:]
            srcidx = rol(_srcidx) if src.name[2] == '0' else flip(_srcidx)
        dstidx = dst.name[1:]
        ...
        if srcidx == dstidx:
            ...
            flit.aux = aux # rol
            return self.channels['c' + str(self.n-aux) + '0' + srcidx]
        else:
            flit.aux = -aux # flip
            return self.channels['c' + str(self.n-aux) + '1' + srcidx]